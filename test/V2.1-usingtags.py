...
merged_movies_rating_tags_df = pd.merge(
    merged_movies_rating_df, tags_df, on=["movieId"], how="outer"
)  # 将打分和电影信息和标签整合到一份表中
merged_movies_rating_tags_df = merged_movies_rating_tags_df.dropna(
    subset=["rating", "title"]
)  # 没有评分或者标题的都滚蛋
merged_movies_rating_tags_df["tag"] = merged_movies_rating_tags_df["tag"].fillna(
    "noTags"
)  # 没有标签也是一种标签

# 处理特征数据
genres_split = merged_movies_rating_tags_df["genres"].str.get_dummies(
    "|"
...
df_unique.drop(columns=["rating"], inplace=True)  # 删除rating列
